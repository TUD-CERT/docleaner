...
async def test_process_pdf_job(
    ...
    jid, job_type = await create_job(sample_pdf, repo, queue, file_identifier)
...
async def test_process_invalid_job(
    ...
    with pytest.raises(ValueError, match=r".*Unsupported document.*"):
        await create_job(b"INVALID_DOCUMENT", repo, queue, file_identifier)
...
async def test_await_again(
    ...
    jid, _ = await create_job(sample_pdf, repo, queue, file_identifier)
